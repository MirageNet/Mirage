...
namespace Mirror.Weaver
{
    static class ServerClientAttributeProcessor
    {
            ...
            foreach (CustomAttribute attr in md.CustomAttributes)
            {
                switch (attr.Constructor.DeclaringType.ToString())
                {
                    ...
                    case "Mirror.ClientCallbackAttribute":
                        ...
                        break;
                    case "Mirror.PlayerAuthorityAttribute":
                        InjectPlayerAuthorityGuard(td, md, true);
                        break;
                    case "Mirror.PlayerAuthorityCallbackAttribute":
                        InjectPlayerAuthorityGuard(td, md, false);
                        break;
        ...
        static void InjectPlayerAuthorityGuard(TypeDefinition td, MethodDefinition md, bool logWarning)
        {
            if (!Weaver.IsNetworkBehaviour(td))
            {
                Weaver.Error($"Player Authority method {md.Name} must be declared in a NetworkBehaviour", md);
                return;
            }
            ILProcessor worker = md.Body.GetILProcessor();
            Instruction top = md.Body.Instructions[0];
            ...
            if (logWarning)
            {
                worker.InsertBefore(top, worker.Create(OpCodes.Ldstr, "[Player Authority] function '" + md.FullName + "' called on player without authority"));
                worker.InsertBefore(top, worker.Create(OpCodes.Call, Weaver.logWarningReference));
            }
