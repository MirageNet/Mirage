    ...
    public class NetworkScenePostProcess : MonoBehaviour
    {
        ...
        public static void OnPostProcessScene()
        {
            ...
            foreach (NetworkIdentity identity in identities)
            {
                ...
                GameObject prefabRootGameObject = prefabGameObject.transform.root.gameObject;
#else
                GameObject prefabRootGameObject = PrefabUtility.FindPrefabRoot(prefabGameObject);
#endif
                if (!prefabRootGameObject)
                    continue;

                if (prefabRootGameObject.GetComponentsInChildren<NetworkIdentity>().Length > 1)
                    Debug.LogWarningFormat("Prefab '{0}' has several NetworkIdentity components attached to itself or its children, this is not supported.", prefabRootGameObject.name);
            }
        }
    }
}
