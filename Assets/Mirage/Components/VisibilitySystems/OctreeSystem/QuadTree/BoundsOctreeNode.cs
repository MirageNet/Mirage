...
namespace Mirage.Components
{
    public struct BoundsOctreeNode<T>
    {
        ...
        public void GetColliding(ref Bounds checkBounds, HashSet<T> result)
        {
            // Are the input bounds at least partially in this node?
            if (!bounds.Intersects(checkBounds))
            {
        ...
        public void GetColliding(ref Ray checkRay, HashSet<T> result, float maxDistance = float.PositiveInfinity)
        {
            // Is the input ray at least partially in this node?
            if (!bounds.IntersectRay(checkRay, out float distance) || distance > maxDistance)
            {
