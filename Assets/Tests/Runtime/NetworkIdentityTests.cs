...
namespace Mirror.Tests
{
    public class NetworkIdentityTests : HostSetup<MockComponent>
    {
        ...
        public IEnumerator DestroyOwnedObjectsTest() => UniTask.ToCoroutine(async () =>
        {
            ...
            await UniTask.WaitUntil(() => !testObj1).Timeout(TimeSpan.FromSeconds(2));
            await UniTask.WaitUntil(() => !testObj2).Timeout(TimeSpan.FromSeconds(2));
            await UniTask.WaitUntil(() => !testObj3).Timeout(TimeSpan.FromSeconds(2));
        });
    }
}
