...
namespace Mirage.Tests.Runtime.Serialization.Packers
{
    public class QuaternionPackerTests : PackerTestBase
    {
        ...
        public void PackAndUnpackRepeat([Range(8, 12)] int bits)
        {
            ...
            Assert.That(outValue.x, IsUnSignedEqualWithIn(inValue.x), $"x off by {Mathf.Abs(assertSign * inValue.x - outValue.x)} q:{inValue.ToString("0.000")}");
            Assert.That(outValue.y, IsUnSignedEqualWithIn(inValue.y), $"y off by {Mathf.Abs(assertSign * inValue.y - outValue.y)} q:{inValue.ToString("0.000")}");
            Assert.That(outValue.z, IsUnSignedEqualWithIn(inValue.z), $"z off by {Mathf.Abs(assertSign * inValue.z - outValue.z)} q:{inValue.ToString("0.000")}");
            Assert.That(outValue.w, IsUnSignedEqualWithIn(inValue.w), $"w off by {Mathf.Abs(assertSign * inValue.w - outValue.w)} q:{inValue.ToString("0.000")}");
        ...
        private Quaternion PackUnpack(Quaternion inValue, QuaternionPacker packer)
        {
            ...
            return packer.Unpack(GetReader());
        }
