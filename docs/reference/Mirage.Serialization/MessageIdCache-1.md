---
id: MessageIdCache-1
title: MessageIdCache<T>
---

# Class MessageIdCache&lt;T&gt;


Class that will cache the ID for type T
avoids needing to calculate the stable hash of the full name each time a message is sent



<div class="inheritance">

##### Inheritance

<div class="level" style={{"--data-index": 0}}>
System.Object
</div>
</div>

##### Inherited Members

<details>
<summary>Show</summary>
</details>

##### Syntax

```cs
public static class MessageIdCache<T>
```

##### Type Parameters
| Name | Description |
| ---- | ---- |
| T |  |


### Fields

#### Id

##### Declaration

```cs
public static readonly int Id
```
#### SendMarker

##### Declaration

```cs
public static readonly ProfilerMarker SendMarker
```
#### SendNotifyMarker

##### Declaration

```cs
public static readonly ProfilerMarker SendNotifyMarker
```
#### SendToManyMarker

##### Declaration

```cs
public static readonly ProfilerMarker SendToManyMarker
```
