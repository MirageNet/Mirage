...
## The Problem
A field marked with `[SyncVar]` specifies a hook name in its attribute, but the hook cannot be resolved, is static (for events), is ambiguous, or does not match the required signature.
