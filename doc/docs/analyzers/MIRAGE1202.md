...
2. **Invalid Return Type:** The method must return `void` or `UniTask<T>`. Returning standard tasks, custom classes, or primitive values directly (without a wrapper) is invalid.
...
2. Ensure the return type is `void` or a valid async task wrapper like `UniTask<T>` (for asynchronous RPCs).
